 ...
 * Premium two-panel AI size finder.
 *   Left  — the product image with measurement callouts + wireframe.
 *   Right — height / weight inputs, a fit profile, and an instant recommendation.
 *
 * Rendered EXACTLY like SizeGuideModal (the proven-working modal on the same
 * page): the parent mounts it only while open and it returns a plain fixed
 * overlay — no portal, no AnimatePresence gating — so it always opens.
 *
 * Props:
 *   onClose       () => void
...
export default function PerfectFitFinder({ onClose, product, sizes: availableSizes = [], onSizeSelect }) {
  ...
  // Close on Escape + lock body scroll (mirrors SizeGuideModal).
  useEffect(() => {
  ...
  return (
    /* Backdrop — plain fixed overlay, same as SizeGuideModal */
    <div
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="grid w-full max-w-[860px] max-h-[92vh] grid-cols-1 overflow-hidden rounded-[28px] bg-white shadow-2xl md:grid-cols-[42%_58%]"
      >
        {/* LEFT — image + callouts */}
        ...
        <div className="relative overflow-y-auto p-6 sm:p-8">
          <button type="button" onClick={onClose} aria-label="Close size finder"
