...
export default function StoreCard({ shop, featuredProducts = [] }) {
  ...
  return (
    <motion.article
      ...
      className="relative flex flex-col rounded-2xl overflow-hidden bg-white border border-[#E8E4DF] shadow-soft cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-espresso focus-visible:ring-offset-2"
      style={{
    ...
    >
      ...
      >
        ...
        {/* Fallback banner when no image — gradient + initials */}
        <div
          className="w-full h-full flex items-center justify-center"
          style={{
            display: bannerSrc ? "none" : "flex",
            background: "linear-gradient(135deg, #1a1a1a 0%, #2C241B 50%, #3a2a1a 100%)",
          }}
        >
          <span className="font-serif text-5xl font-bold opacity-20" style={{ color: "#C9A84C" }}>
        ...
        <div className="flex items-start gap-3 -mt-8 relative z-10">
          ...
          >
            ...
            <span
              ...
              style={{ display: (shop?.image_url || shop?.logo) ? "none" : "flex", color: "#2C241B" }}
              aria-hidden="true"
            >
              {getInitials(shop?.name)}
            </span>
          ...
          <div className="flex flex-col pt-5 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              {/* Source label */}
              {shop?.is_admin_store || shop?.source === "exclusive" ? (
