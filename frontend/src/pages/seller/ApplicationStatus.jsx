            ...
            <section className="bg-gray-50 py-12 px-4 min-h-[60vh]">
                <div className="max-w-2xl mx-auto space-y-5">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
                        ...
                        {/* Approved — show portal access */}
                        {app.status === "approved" && (
                            <div className="bg-green-50 rounded-2xl border border-green-200 p-6">
                                <p className="text-xs font-bold uppercase tracking-widest text-green-700 mb-3">🎉 Your Seller Account is Ready</p>
                                <p className="text-sm text-gray-700 mb-4 leading-relaxed">
                                    Log in to your Seller Portal using your registered email address.
                                    Use the password you set during registration, or{" "}
                                    <span className="font-semibold">seller123</span> if this is a demo account.
                                </p>
                                <div className="bg-white rounded-xl p-4 mb-4 border border-green-100 space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-gray-500">Email</span>
                                        <span className="font-mono font-semibold text-[#0a0a0a]">{app.applicant_email}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-500">Role</span>
                                        <span className="font-semibold text-green-700">Verified Seller</span>
                                    </div>
                                </div>
                            </div>
                        )}
                        <div className="flex flex-col sm:flex-row gap-3 mt-4">
                            {app.status === "approved" && (
                                <Link to="/seller/login"
                                    className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl hover:opacity-90 transition font-semibold text-sm"
                                    style={{ backgroundColor: "#1a1a1a", color: "#ffffff" }}>
                                    Go to Seller Portal <ArrowRight size={15} color="#ffffff" />
                                </Link>
                            )}
                            ...
                            <Link to="/"
                                className="flex-1 flex items-center justify-center px-6 py-3 rounded-xl transition font-medium text-sm"
                                style={{ border: "1px solid #E8E4DF", color: "#1a1a1a" }}>
                                Back to Home
                            </Link>
                        </div>
                    </motion.div>
