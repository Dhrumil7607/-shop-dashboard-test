...
export default function AdminBookings() {
    ...
    // Load bookings from backend only (source of truth)
    useEffect(() => {
        const adminKey = localStorage.getItem("slb_admin_key") || "shoplivebharat-admin";
        api.get("/admin/bookings", { headers: { "X-Admin-Key": adminKey } })
            .then(res => {
                ...
                const normalized = data.map(b => ({
                    ...
                    consultantName: b.shop_name || b.store_name || b.consultantName || "",
                    shopName: b.shop_name || b.store_name || b.shopName || "",
                    storeName: b.shop_name || b.store_name || "",
                    price: b.session_fee || b.price || 0,
                    sessionFee: b.session_fee || b.sessionFee || 0,
                    paymentStatus: b.payment_status || b.paymentStatus || "pending",
                    googleMeetLink: b.google_meet_link || b.googleMeetLink || null,
                }));
    ...
    const handleStatusChange = (newStatus) => {
        if (selectedBooking) {
            const bid = selectedBooking.id || selectedBooking.bookingId;
            const adminKey = localStorage.getItem("slb_admin_key") || "shoplivebharat-admin";
            api.patch(`/bookings/${bid}`, { status: newStatus }, { headers: { "X-Admin-Key": adminKey } })
                .then(() => {
                    setBookings((prev) => prev.map((b) => (b.id || b.bookingId) === bid ? { ...b, status: newStatus } : b));
    ...
    const handleRowUpdate = (bookingId, patch) => {
        const adminKey = localStorage.getItem("slb_admin_key") || "shoplivebharat-admin";
        api.patch(`/bookings/${bookingId}`, patch, { headers: { "X-Admin-Key": adminKey } })
            .then(() => {
                setBookings((prev) => prev.map((b) => (b.id || b.bookingId) === bookingId ? { ...b, ...patch } : b));
            })
            .catch(() => toast.error("Failed to update"));
    };
