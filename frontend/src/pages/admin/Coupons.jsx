...
// Map a backend coupon (snake_case) to the shape this UI renders (camelCase)
const _fromApi = (c) => ({
    id: c.id,
    code: c.code,
...
export default function AdminCoupons() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editId) {
                await updateAdminCoupon(editId, payload, adminKey);
                ...
                await createAdminCoupon(payload, adminKey);
            ...
            const msg = err?.response?.data?.detail || "Could not save coupon";
            if (typeof msg === "string" && msg.toLowerCase().includes("code")) setErrors({ code: msg });
            else toast.error(typeof msg === "string" ? msg : "Could not save coupon");
        }
    };

    const handleDelete = async (id) => {
        try {
            await deleteAdminCoupon(id, adminKey);
            await loadCoupons();
    ...
    const toggleActive = async (id) => {
        ...
        if (!coupon) return;
        try {
            await updateAdminCoupon(id, { active: !coupon.active }, adminKey);
            await loadCoupons();
        } catch {
