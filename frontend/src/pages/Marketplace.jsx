...
export default function Marketplace() {
    ...
    // Re-fetch on window focus AND on navigation
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const data = await fetchProducts({ active_only: true, limit: 500 });
    ...
    const filtered = useMemo(() => {
        let result = [...products];
        // ── Dedupe by name + shop (removes catalog duplicates) ──
        const seen = new Set();
        result = result.filter(p => {
            const key = `${(p.name || "").toLowerCase().trim()}|${p.shop_id || p.shop_name || ""}`;
        ...
        <MarketplaceLayout>
            ...
            <div className="px-4 md:px-12 pb-16" style={{ backgroundColor: "#FAF9F6" }}>
                <div className="max-w-7xl mx-auto">
                    ...
                    {/* ── Mobile Filter Bar (stacked: search on top, filters + sort below) ── */}
                    <div className="flex flex-col gap-3 mb-4 lg:hidden">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-3 py-3 text-sm rounded-xl border outline-none min-h-[48px]"
                                style={{ backgroundColor: "white", border: "1px solid #E8E4DF", color: "#1a1a1a" }}
                        ...
                        <div className="flex items-center gap-3">
                            <button
                                ...
                                className="flex items-center justify-center gap-2 flex-1 px-4 py-3 rounded-xl border text-sm font-semibold min-h-[48px]"
                            ...
                            <div className="relative flex-1">
                                <select value={sort} onChange={e => setSort(e.target.value)}
                                    className="appearance-none w-full pl-4 pr-9 py-3 text-sm rounded-xl border outline-none cursor-pointer min-h-[48px]"
                                    style={{ backgroundColor: "white", border: "1px solid #E8E4DF", color: "#1a1a1a" }}
