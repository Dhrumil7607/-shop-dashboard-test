...
export function CartProvider({ children }) {
    ...
    const addToCart = useCallback((product) => {
        setCartItems(prev => {
            ...
            if (existing) {
                toast.success("Shopping Bag Updated", { description: `${product.name || "Item"} quantity increased.` });
                return prev.map(item =>
                    item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item
                );
            }
            toast.success("Added to Your Shopping Bag", { description: `${product.name || "Item"} has been added successfully.` });
    ...
    const removeFromCart = useCallback((productId) => {
        ...
        toast.success("Removed from Your Bag", { description: "Item removed from your shopping bag." });
    }, []);
