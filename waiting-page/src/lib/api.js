...
export async function createShop(payload, adminKey) {
    console.log("🔑 Creating shop with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
    console.log("📤 Config:", config);
    const { data } = await api.post("/admin/shops", payload, config);
    return data;
}

export async function updateShop(id, payload, adminKey) {
    console.log("🔑 Updating shop with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
    const { data } = await api.patch(`/admin/shops/${id}`, payload, config);
    return data;
}

export async function archiveShop(id, adminKey) {
    console.log("🔑 Archiving shop with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
    const { data } = await api.delete(`/admin/shops/${id}`, config);
    return data;
}

export async function createProduct(payload, adminKey) {
    console.log("🔑 Creating product with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
...
export async function updateProduct(id, payload, adminKey) {
    console.log("🔑 Updating product with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
    const { data } = await api.patch(`/admin/products/${id}`, payload, config);
    return data;
}

export async function archiveProduct(id, adminKey) {
    console.log("🔑 Archiving product with admin key:", adminKey ? "present" : "missing");
    const config = adminHeaders(adminKey);
    const { data } = await api.delete(`/admin/products/${id}`, config);
    return data;
}
